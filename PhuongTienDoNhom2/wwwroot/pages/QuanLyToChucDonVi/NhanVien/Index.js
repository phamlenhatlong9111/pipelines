...
$(document).ready(function () {
    ...
    $('#dataGrid').DataTable({
        ...
        "ajax": {
            url: `${APIURL}/api/TDC_NhanVienKDHCTNApi/danhsach`,
            type: "GET",
            dataSrc: function (data) {
                if (data.isSuccess) {
                    ...
                    result.map((item, index) => {
                        item.stt = index + 1
                        item.loaiHopDong = item.vienChuc ? "Viên chức" : "Người lao động"
                        item.namSinh = formatDate(new Date(item.namSinh))
                        item.ngayCap = formatDate(new Date(item.ngayCap))
                        item.ngayHetHan = formatDate(new Date(item.ngayHetHan))
                        item.trangThaiF = item.trangThai == true ? "Còn làm việc" : "Đã nghỉ làm việc"
                        if (item.gioiTinh == 0) {
                            item.gioiTinh = 'Nữ'
                        }
        ...
        "columns": [
            ...
            { data: "hoTen", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "gioiTinh", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "namSinh", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "noiCuTru", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "loaiHopDong", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "soHieuKDV", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "trangThaiF", "width": "auto", "class": "left-align dnCustom", searchable: true },
            { data: "maNhanVien", "width": "120px", "class": "center-align nowrap group-icon-action stt-text" },
        ]
    ...
    $('#btn-save-Add').on('click', function () {
        ...
        let gioiTinh = $('#modalAdd input[name="fav_GioiTinh"]:checked').val();
        let namSinhF = $('#namSinhAdd').val()
        let namSinh = formatDateSql(namSinhF)
        let nguyenQuan = $('#nguyenQuanAdd').val()
        let noiCuTru = $('#noiCuTruAdd').val()
        let vienChuc = $('#loaiHDVC_True').is(':checked')
        let trinhDo = $('#trinhDoHocVanAdd').val()
        let kinhNghiem = $('#kinhNghiemLamViecAdd').val()
        let soHieuKDV = $('#soHieuKDVAdd').val()
        let noiCap = $('#noiCapAdd').val()
        ...
        let trangThai = $('#trangThaiLV_True').is(':checked')
        let dataBuilder = {
            "maToChuc": maToChuc,
            "hoTen": hoTen,
            "gioiTinh": gioiTinh,
            "namSinh": namSinh ? namSinh : null,
            "nguyenQuan": nguyenQuan ? nguyenQuan : null,
            "noiCuTru": noiCuTru ? noiCuTru : null,
            "vienChuc": vienChuc,
            "trinhDo": trinhDo ? trinhDo : null,
            "kinhNghiem": kinhNghiem ? kinhNghiem : null,
            "soHieuKDV": soHieuKDV ? soHieuKDV : null,
            "noiCap": noiCap ? noiCap : null,
        ...
        if (checkEmptyBlank(hoTen) || checkEmptyBlank(gioiTinh) || checkEmptyBlank(vienChuc) || checkEmptyBlank(soHieuKDV)) {
            showNotify("Các trường bắt buộc không được để trống", "danger")
        }
        else {
            if (!checkTagHtml(hoTen)) {
                showNotify("Họ tên chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(nguyenQuan)) {
                showNotify("Nguyên quán chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(noiCuTru)) {
                showNotify("Nơi cư trú chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(trinhDo)) {
                showNotify("Trình độ học vấn chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(kinhNghiem)) {
                showNotify("Kinh nghiệm công tác chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(soHieuKDV)) {
                showNotify("Số hiệu kiểm định viên chứa kí tự không hợp lệ", "danger")
            }
            else if (!checkTagHtml(noiCap)) {
                showNotify("Tổ chức cấp GCNKĐV chứa kí tự không hợp lệ", "danger")
            }
            else if (new Date(ngayHetHan) < new Date(ngayCap)) {
                showNotify("Ngày hết hạn không hợp lệ", "danger")
            }
            else {
                if (resultCheck == 0) {
                    if (checkFile == 1) {
                        ...
                        $.ajax({
                            type: 'post',
                            async: false,
                            url: `${APIURL}/api/TDC_NhanVienKDHCTNApi/themmoi`,
                            processData: false,
                            contentType: false,
                            data: formData,
                    ...
                    showNotify("Số ký hiệu của kiểm định viên đã tồn tại", "danger")
                }
            }
        }
    })
    ...
    let resetModalAdd = () => {
        ...
        $('#modalAdd input[type="radio"]').prop('checked', false);
...
const kiemTraSoKyHieu = (dataBuilder) => {
    ...
    $.ajax({
        type: 'post',
        async: false,
        url: `${APIURL}/api/TDC_NhanVienKDHCTNApi/kiemtra`,
        contentType: 'application/json;charset=utf-8',
