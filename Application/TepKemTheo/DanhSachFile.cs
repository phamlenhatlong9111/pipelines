            ...
            public async Task<Result<IEnumerable<Domain.TepKemTheo>>> Handle(Query request, CancellationToken cancellationToken)
            {
                using (var connettion = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        parameters.Add("@PIDDOITUONG", request.IdDoiTuong);
                        var result = await connettion.QueryAsync<Domain.TepKemTheo>("SP_TEPKEMTHEO_DANHSACHFILETHEODOITUONG", parameters, commandType: System.Data.CommandType.StoredProcedure);
