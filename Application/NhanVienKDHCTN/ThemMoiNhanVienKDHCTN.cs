...
{
    public class ThemMoiNhanVienKDHCTN
    {
        public class Command : IRequest<Result<PTD_NhanVienKDHCTN>>
        {
            public PTD_NhanVienKDHCTNAddModel Entity { get; set; }
        }
            ...
            public async Task<Result<PTD_NhanVienKDHCTN>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        var result = await connection.QueryFirstOrDefaultAsync<PTD_NhanVienKDHCTN>("TDC_Add_NhanVienKDHCTN", parameters, commandType: System.Data.CommandType.StoredProcedure);
