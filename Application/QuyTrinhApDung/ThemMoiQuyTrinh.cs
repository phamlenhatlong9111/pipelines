...
{
    public class ThemMoiQuyTrinh
    {
        public class Command : IRequest<Result<PTD_QuyTrinhApDung>>
        {
            public PTD_QuyTrinhApDungAddModel Entity { get; set; }
        }
            ...
            public async Task<Result<PTD_QuyTrinhApDung>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        var result = await connection.QueryFirstOrDefaultAsync<PTD_QuyTrinhApDung>("TDC_Add_QuyTrinhApDung", parameters, commandType: System.Data.CommandType.StoredProcedure);
