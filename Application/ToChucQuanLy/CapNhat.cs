            ...
            public async Task<Result<PTD_ToChucQuanLy>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (var connettion = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        var result = await connettion.QueryFirstOrDefaultAsync<PTD_ToChucQuanLy>("TDC_Update_ToChucQuanLy", parameters, commandType: System.Data.CommandType.StoredProcedure);
