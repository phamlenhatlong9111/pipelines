...
{
    public class CapNhatToChuc
    {
        public class Command : IRequest<Result<PTD_ToChucKDHCTN>>
        {
            public PTD_ToChucKDHCTN Entity { get; set; }
        }
            ...
            public async Task<Result<PTD_ToChucKDHCTN>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (var connettion = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        var result = await connettion.QueryFirstOrDefaultAsync<PTD_ToChucKDHCTN>("TDC_ToChucKDHCTN_UPDATE", parameters, commandType: System.Data.CommandType.StoredProcedure);
